...
public class autoScore extends Command {
  ...
  public void execute() {
    ...
    if (goalPose.getX() < 0.07) {
        if (elevatorPos == 4) {
            elevator.elevatorL4();
            if (elevator.getPosition() > ElevatorConstants.L4Pos - 15) {
                coral.justScore();
                myTimer.start();
                if (myTimer.hasElapsed(1)) {
                    coral.stop();
        ...
        if (elevatorPos == 3) {
            elevator.elevatorL3();
            if (elevator.getPosition() > ElevatorConstants.L3Pos - 15) {
                coral.justScore();
                myTimer.start();
                if (elapsedTime == 1) {
                    coral.stop();
        ...
        if (elevatorPos == 2) {
            elevator.elevatorL2();
            if (elevator.getPosition() > ElevatorConstants.L2Pos - 15) {
                coral.justScore();
                myTimer.start();
                if (elapsedTime == 1) {
                    coral.stop();
